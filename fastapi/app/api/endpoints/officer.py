...
def get_review_queue(
    ...
    current_user: User = Depends(require_roles([Role.OFFICER.value, Role.ADMIN.value])),
...
async def create_qr_payment(user_id: int, db: Session = Depends(get_db)):
    ...
    if outstanding_balance <= 0:
        raise HTTPException(
            status_code=400, detail="Không có khoản phạt nào cần thanh toán"
        )

    payment = await processor.create_qr_payment(
        user_id=user_id, amount=outstanding_balance
    )
