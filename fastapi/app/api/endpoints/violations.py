    ...
    if current_user.role == Role.CITIZEN:
        ...
        if not plates:
            return ViolationListResponse(
                violations=[], total=0, page=skip // limit + 1, size=limit
            )
        ...
        total = violation_service.count_violations(
            license_plate=effective_license_plate, status=status
        )
    ...
    for violation in violations:
        ...
        if violation.video_id:
            video = (
                ...
                .filter(CameraVideo.id == violation.video_id)
                .first()
            )
            if video:
...
def get_recent_violations(
    ...
    violations = (
        db.query(Violation).order_by(Violation.detected_at.desc()).limit(limit).all()
    )
        ...
        result.append(
            {
                ...
                "time": v.detected_at.strftime("%d/%m/%Y %H:%M")
                if v.detected_at
                else "N/A",
                "status": ViolationStatus.PROCESSED.value
                if v.status
                in [ViolationStatus.APPROVED.value, ViolationStatus.PAID.value]
                else ViolationStatus.PENDING.value,
            }
        )
...
def get_processed_violations(
    ...
    # Get violations with 'processed' status
    violations = violation_service.search_violations(
        status=ViolationStatus.PROCESSED.value,
        skip=skip,
        limit=limit,
    ...
    # Get total count
    total = violation_service.count_violations(status=ViolationStatus.PROCESSED.value)
...
def get_violation_detail(
    ...
    if current_user.role == Role.CITIZEN.value:
        ...
        if violation.license_plate not in my_license_plates:
            raise HTTPException(
                status_code=403, detail="Không có quyền xem vi phạm này"
            )
    ...
    if violation.video_id:
        video = (
            db.query(CameraVideo).filter(CameraVideo.id == violation.video_id).first()
        )
        if video:
            response_data.video_evidence = VideoEvidenceInfo(
                video_id=video.id,
