...
class ComplaintService:
    ...
    def get_complaint_by_id(self, complaint_id: int) -> Complaint:
        ...
        if not complaint:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Khiếu nại không tồn tại"
            )
        return complaint
    ...
    def create_complaint(self, complaint_data: dict, user_id: int) -> Complaint:
        ...
        complaint_data["description"] = sanitize_input(
        ...
        if not user:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Chưa đăng nhập"
            )
    ...
    def get_user_complaints(self, user_id: int) -> List[Complaint]:
        ...
        return (
            self.db.query(Complaint)
            .filter(Complaint.complainant_id == user_id)
    ...
    def get_assigned_complaints(self, officer_id: int) -> List[Complaint]:
        ...
        return (
            ...
            .order_by(Complaint.assigned_at.desc())
            .all()
        )
    ...
    def assign_complaint(
        self, complaint_id: int, officer_id: int, assigned_by: int
    ) -> Complaint:
    ...
    def resolve_complaint(
        self, complaint_id: int, resolution: str, resolved_by: int
    ) -> Complaint:
        ...
        # Log activity
        self._log_activity(
            complaint.id, "resolved", "Khiếu nại đã được giải quyết", resolved_by
        )
    ...
    def create_appeal(self, complaint_id: int, appeal_data: dict) -> ComplaintAppeal:
        ...
        today = datetime.now().date()
        count_today = (
            self.db.query(ComplaintAppeal)
            .filter(
                ComplaintAppeal.created_at
    ...
    def get_complaint_activities(self, complaint_id: int) -> List[ComplaintActivity]:
        ...
        return (
            self.db.query(ComplaintActivity)
            .filter(ComplaintActivity.complaint_id == complaint_id)
            .order_by(ComplaintActivity.performed_at.asc())
            .all()
        )
    ...
    def get_complaint_statistics(
        ...
        total_complaints = (
            self.db.query(Complaint)
            .filter(Complaint.created_at.between(start_date, end_date))
            .count()
        )
    ...
    def rate_complaint(
        self, complaint_id: int, rating: int, feedback: Optional[str] = None
    ) -> Complaint:
        ...
        # Log activity
        self._log_activity(
            complaint_id, "rated", f"Đánh giá {rating} sao", complaint.complainant_id
        )
    ...
    def _calculate_average_rating(
        ...
        ratings = (
            self.db.query(Complaint.user_rating)
            .filter(
                Complaint.created_at.between(start_date, end_date),
