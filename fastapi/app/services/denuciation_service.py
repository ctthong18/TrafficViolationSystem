...
class DenunciationService:
    ...
    def get_denunciation_by_id(self, denunciation_id: int) -> Denunciation:
        ...
        denunciation = (
            self.db.query(Denunciation)
            .filter(Denunciation.id == denunciation_id)
            .first()
        )
        if not denunciation:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Tố cáo không tồn tại"
            )
        return denunciation
    ...
    def create_denunciation(self, denunciation_data: dict) -> Denunciation:
        ...
        denunciation_data["description"] = sanitize_input(
            denunciation_data.get("description", "")
        )
    ...
    def update_denunciation_status(
        self,
        denunciation_id: int,
        ...
        self._log_activity(
            denunciation.id, "status_updated", activity_desc, investigator_id
        )

        return denunciation

    def resolve_denunciation(
        self, denunciation_id: int, resolution: str, investigator_id: int
    ) -> Denunciation:
        """Giải quyết tố cáo"""
        denunciation = self.get_denunciation_by_id(denunciation_id)
        ...
        # Log activity
        self._log_activity(
            denunciation.id, "resolved", "Tố cáo đã được giải quyết", investigator_id
        )

        return denunciation

    def transfer_denunciation(
        self, denunciation_id: int, transfer_to: str, reason: str, investigator_id: int
    ) -> Denunciation:
        ...
        self._log_activity(
            denunciation.id, "transferred", activity_desc, investigator_id
        )

        return denunciation

    def get_pending_denunciations(
        self, skip: int = 0, limit: int = 50
    ) -> List[Denunciation]:
        """Lấy danh sách tố cáo đang chờ xử lý"""
        return (
            self.db.query(Denunciation)
            .filter(Denunciation.status == DenunciationStatus.PENDING)
            .order_by(Denunciation.created_at.desc())
            .offset(skip)
            .limit(limit)
            .all()
        )
    ...
    def get_assigned_denunciations(self, investigator_id: int) -> List[Denunciation]:
        ...
        return (
            self.db.query(Denunciation)
            .filter(Denunciation.assigned_investigator_id == investigator_id)
            .order_by(Denunciation.assigned_at.desc())
            .all()
        )

    def search_denunciations(
        self,
        ...
        if accused_person:
            query = query.filter(
                Denunciation.accused_person_name.ilike(f"%{accused_person}%")
            )
        if start_date:
        ...
        return (
            query.order_by(Denunciation.created_at.desc())
            .offset(skip)
            .limit(limit)
            .all()
        )

    def get_denunciation_statistics(
        self, start_date: datetime, end_date: datetime
    ) -> Dict[str, Any]:
        """Thống kê tố cáo theo khoảng thời gian"""
        total_denunciations = (
            self.db.query(Denunciation)
            .filter(Denunciation.created_at.between(start_date, end_date))
            .count()
        )
        ...
        by_severity = (
            self.db.query(Denunciation.severity_level, Denunciation.status)
            .filter(Denunciation.created_at.between(start_date, end_date))
            .all()
        )
        ...
        return {
            ...
            "resolution_rate": by_status.get("resolved", 0) / total_denunciations
            if total_denunciations > 0
            else 0,
    ...
    def get_whistleblower_protection_stats(self) -> Dict[str, Any]:
        ...
        total_anonymous = (
            self.db.query(Denunciation)
            .filter(Denunciation.is_anonymous == True)
            .count()
        )

        total_whistleblowers = (
            self.db.query(Denunciation)
            .filter(Denunciation.is_whistleblower == True)
            .count()
        )

        high_security_cases = (
            self.db.query(Denunciation)
            .filter(Denunciation.security_level.in_(["secret", "top_secret"]))
            .count()
        )
        ...
        return {
            ...
            "anonymous_rate": total_anonymous / self.db.query(Denunciation).count()
            if self.db.query(Denunciation).count() > 0
            else 0,
        }
    ...
    def get_denunciation_activities(
        self, denunciation_id: int
    ) -> List[DenunciationActivity]:
        """Lấy lịch sử hoạt động của tố cáo"""
        return (
            self.db.query(DenunciationActivity)
            .filter(DenunciationActivity.denunciation_id == denunciation_id)
            .order_by(DenunciationActivity.performed_at.asc())
            .all()
        )

    def get_user_denunciations(
        self, user_identification: str, user_email: str = None
    ) -> List[Denunciation]:
        ...
        if user_identification:
            conditions.append(
                Denunciation.informant_identification == user_identification
            )
        if user_email:
    ...
    def export_denunciations_report(
        self, start_date: datetime, end_date: datetime
    ) -> List[Dict[str, Any]]:
        """Xuất báo cáo tố cáo"""
        denunciations = (
            ...
            report.append(
                {
                    ...
                    "assigned_investigator": denunciation.assigned_investigator.full_name
                    if denunciation.assigned_investigator
                    else None,
