...
class VehicleService:
    ...
    def create_vehicle(self, vehicle_data: VehicleCreate, user_id: int) -> Vehicle:
        # Convert dict to VehicleCreate if necessary
        if isinstance(vehicle_data, dict):
            vehicle_data = VehicleCreate(**vehicle_data)
        ...
        vehicle = Vehicle(**vehicle_data.dict(), owner_id=user_id)
    ...
    def delete_vehicle(self, vehicle_id: int, user_id: int) -> None:
        ...
        violation_count = (
            ...
            .filter(Violation.license_plate == vehicle.license_plate)
            .count()
        )
    ...
    def _get_last_violation_date(self, license_plate: str):
        ...
        last_violation = (
            self.db.query(Violation)
            .filter(Violation.license_plate == license_plate)
