...
class ViolationService:
    ...
    def get_violation_by_id(self, violation_id: int) -> Violation:
        ...
        if not violation:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Vi phạm không tồn tại"
            )
        return violation
    ...
    def get_pending_violations(
        self, skip: int = 0, limit: int = 50, priority: Optional[str] = None
    ) -> List[Violation]:
        ...
        query = self.db.query(Violation).filter(
            Violation.status.in_(
                [ViolationStatus.PENDING.value, ViolationStatus.REVIEWING.value]
            )
        )
        ...
        return (
            query.order_by(Violation.detected_at.desc()).offset(skip).limit(limit).all()
        )
    ...
    def review_violation(
        ...
        if violation.status not in [
            ViolationStatus.PENDING.value,
            ViolationStatus.REVIEWING.value,
        ]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="Vi phạm đã được xử lý"
            )
        ...
        if normalized in ["approve", "approved", "verify", "verified"]:
            violation.status = ViolationStatus.VERIFIED.value
        elif normalized in ["reject", "rejected", "process", "processed"]:
            violation.status = ViolationStatus.PROCESSED.value
        elif normalized in ["processing", "reviewing"]:
            violation.status = ViolationStatus.REVIEWING.value
        else:
            raise HTTPException(
        ...
        if is_new_assignment:
            try:
                self.notification_service.notify_officer_assignment(
                    violation_id=violation_id, officer_id=officer_id
                )
    ...
    def get_violations_by_license_plate(
        self, license_plate: str, status: Optional[str] = None
    ) -> List[Violation]:
        query = self.db.query(Violation).filter(
    ...
    def get_violations_by_vehicles(
        self, license_plates: List[str], status: Optional[str] = None
    ) -> List[Violation]:
        query = self.db.query(Violation).filter(
            Violation.license_plate.in_(license_plates)
        )
    ...
    def get_violation_statistics(
        ...
        total_violations = (
            ...
            .filter(Violation.detected_at.between(start_date, end_date))
            .count()
        )
        ...
        rejected_violations = (
            ...
            .filter(
                Violation.detected_at.between(start_date, end_date),
                Violation.status == ViolationStatus.REJECTED.value,
            )
            .count()
        )
        ...
        # Violations by type
        violations_by_type = (
            self.db.query(Violation.violation_type, Violation.status)
            .filter(Violation.detected_at.between(start_date, end_date))
            .all()
        )
        ...
        for violation_type, status in violations_by_type:
            ...
            if status == ViolationStatus.APPROVED.value:
                type_stats[violation_type]["approved"] += 1
            elif status == ViolationStatus.REJECTED.value:
        ...
        return {
            ...
            "approval_rate": approved_violations / total_violations
    ...
    def get_officer_stats(self, officer_id: int) -> Dict[str, Any]:
        ...
        total_reviewed = (
            self.db.query(Violation).filter(Violation.reviewed_by == officer_id).count()
        )
        ...
        processing_times = (
            self.db.query(Violation)
            .filter(
                Violation.reviewed_by == officer_id, Violation.reviewed_at.isnot(None)
            )
            .with_entities(Violation.created_at, Violation.reviewed_at)
            .all()
        )
        ...
        return {
            ...
            "efficiency_rate": (approved_today + rejected_today) / 8
            if (approved_today + rejected_today) > 0
            else 0,  # Assuming 8-hour workday
            "average_processing_time": avg_processing_time,
            "pending_assigned": self.db.query(Violation)
            .filter(Violation.status == ViolationStatus.PENDING.value)
            .count(),  # All pending, not officer-specific
        }

    def batch_update_violations(
        self, violation_ids: List[int], update_data: Dict[str, Any]
    ) -> int:
        """Batch update multiple violations"""
        updated_count = (
            self.db.query(Violation)
            .filter(Violation.id.in_(violation_ids))
    ...
    def get_processed_violations(
        ...
        processed_statuses = [
            ViolationStatus.VERIFIED.value,
            ViolationStatus.PROCESSED.value,
            ViolationStatus.PAID.value,
        ]
        return (
            self.db.query(Violation)
            .filter(Violation.status.in_(processed_statuses))
    ...
    def get_user_violation_stats(self, license_plates: List[str]) -> Dict[str, Any]:
        ...
        if not license_plates:
            return {"total": 0, "unpaid": 0, "total_fines": 0.0}
        ...
        unpaid = len(
            [
                ...
                for v in violations
                if v.status
                not in [ViolationStatus.PAID.value, ViolationStatus.PROCESSED.value]
            ]
        )
        total_fines = sum(float(v.fine_amount or 0) for v in violations)

        return {"total": total, "unpaid": unpaid, "total_fines": total_fines}
